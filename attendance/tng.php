				...
				else
				{
					...
					if(count($res) != 0)
					{
						...
						else
						{
							...
							echo "<center><h3>Defaulters during the specified period</h3></center>";
							$table = new html_table();
							//$table->tablealign = 'center';
							$table->attributes['class'] = 'generaltable';
							$table->head = array("PRN","Name","Attendance during the specified period");
							asort($blockStudentsData);
							$table->data = $blockStudentsData;
				...
				foreach($studentAtt as $key => $entry)
				{
					if(ceil($entry) < 75 && in_array($key,array_keys($_SESSION['blockedStudentIds'])))
					{
						...
						if(count($defaulters) != 0)
						{
							...
							{	
								$name=$entry->username;
								$defaultersList[] = array($name);
								$fullnames[] = $entry->firstname." ".$entry->lastname;
							}
							$data = $defaultersList;
							$i = 0;
							foreach($defaultersList as $key=>$value)
							{
								$name = $value[0];
								foreach($usernames as $subkey=>$users)
								{
									if($users->username == $name)
									{
										$defaultersList[$key][] = $fullnames[$i++];
										$defaultersList[$key][] = $defaulters[$subkey];
									}
								}
							}
							
							echo "<center><h3>Defaulters</h3></center>";
							$table = new html_table();
							//$table->tablealign = 'center';
							$table->attributes['class'] = 'generaltable';
							$table->head = array("PRN","Name","Consolidated Attendance Percentage");
							asort($defaultersList);
							$table->data =  $defaultersList;
							echo html_writer::table($table);
