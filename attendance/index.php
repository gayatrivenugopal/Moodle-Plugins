...
class Settings_Form extends moodleform {
	...
	public function set_elements()
	{
		...
		$this->mform->addElement('submit', 'block', get_string('view','local_attendance'));
	}
...
if($mform->get_data() != null) //true if user clicked "Block Defaulters"
{
	...
	$studentAtt = array();
	$studentTotalSessions = array();
	$studentSessionsAttended = array();
					...
					if(count($res) != 0)
					{
						...
						{	
							foreach($studentMaxGrades as $keyMax=>$val)
							{
								if($keyMax == $keyCurr)
								{
									...
									$totalSessions[$keyCurr] = $val;
									$sessionsAttended[$keyCurr] = $entry;
								}
							}
						}
						foreach($studentAtt as $key => $entry)
						{
							/*if($entry < $mform->get_data()->minatt+1)
							{
						...
						else
						{
							...
							$data = $tableData;
								...
								$table->head = array(get_string('studentsblocked', 'local_attendance'),get_string('perct','local_attendance'));
								...
								if(count($safeStudentIds) != 0)
								{
									...
									foreach($safeStudentsData as $key=>$value)
									{
										...
										foreach($usernames as $subkey=>$users)
										{
											if($users->username == $name)
											{
												$safeStudentsData[$key][] = $fullnames[$i++];
												$safeStudentsData[$key][] = $safeStudentIds[$subkey];
												$safeStudentsData[$key][] = $sessionsAttended[$subkey];
												$safeStudentsData[$key][] = $totalSessions[$subkey];
											}
										}
									}
									$table = new html_table();
									//$table->tablealign = 'center';
									$table->attributes['class'] = 'generaltable';
									$table->head = array("PRN", "Name", get_string('perct','local_attendance'), "Sessions Attended", "Total Sessions");
									asort($safeStudentsData);
...
function exportToTable($filename, $format) {
	...
	header("Location:$CFG->wwwroot/local/attendance/download.php?format=".$format."&filename=".$filename);
}
...
/*function exportToText($filename)
{
	...
	fputcsv($fp,array("PRN","Name","Attendance"));
	...
	fclose($fp);	
	header("Location:$CFG->wwwroot/local/consolidatedattendance/download.php?format=text&filename=".$filename);
}*/
?>
