...
function insertIntoTable($users, $from, $to)
{
	...
	{	
		...
		//echo $queryString;
		//$result = $DB->get_records_select("mdl_blockdefaulters",$select);//$DB->count_records_sql($queryString);
			...
			if(count($attId) != 0)
			{
				...
				else
				{
					...
					foreach($studentId as $keyStudent=>$entry)
					{
						...
						{
							if($statusId[$keyStudent] == $key && ($statusArray[$key][$key]->acronym != "x" && $statusArray[$key][$key]->acronym != "X"))
							{
								if($studentMaxGrades[$entry] == null)
