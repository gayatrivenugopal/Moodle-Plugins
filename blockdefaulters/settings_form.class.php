...
class Settings_Form extends moodleform {
    ...
    public function definition() {
		...
		//	sort($this->keys);
			$select = $mform->addElement('select', 'programme', get_string('programmes','local_blockdefaulters'), array_values($categories));
			...
			$select = $mform->addElement('select', 'minatt', get_string('mincriterion','local_blockdefaulters'), $this->perct);
			$select->setSelected(74);
			
			$mform->addElement('date_time_selector', 'considerfrom', get_string('considerfrom','local_blockdefaulters'));
			$mform->addElement('date_time_selector', 'considertill', get_string('considertill','local_blockdefaulters'));
			$mform->addElement('submit', 'block', get_string('blockdefaulters','local_blockdefaulters'));
	}
